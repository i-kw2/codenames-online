(function () {
  ...
  H.test('operative can end turn with zero guesses after a clue', () => {
    G.__test.replaceState(baseState());
  ...
  H.test('candidate can be selected then confirmed as the final guess', () => {
    G.__test.replaceState(baseState());
    const selected = G.toggleCandidate('r1', 'p1');
    H.ok(selected.ok); H.ok(selected.selected);
    H.ok(G.getState().candidateSelection.cardIds.includes('r1'));
    const confirmed = G.commitCandidate('r1', 'p1');
    H.ok(confirmed.ok);
    H.equal(G.getState().remaining.red, 1);
    H.ok(!G.getState().candidateSelection.cardIds.includes('r1'));
  });

  H.test('candidate selection survives Firebase-style missing empty cardIds', () => {
    const state = baseState();
    state.currentClue.submittedAt = '2026-09-10T00:00:00.000Z';
    // Firebase RTDB can omit an empty [] from an object after a round-trip.
    // قد يحذف Firebase المصفوفة الفارغة من candidateSelection.
    state.candidateSelection = {
      team: 'red',
      clueSubmittedAt: state.currentClue.submittedAt,
      updatedAt: '2026-09-10T00:00:01.000Z',
      updatedBy: null
    };
    G.replaceState(state, false);
    H.ok(Array.isArray(G.getState().candidateSelection.cardIds));
    const selected = G.toggleCandidate('r1', 'p1');
    H.ok(selected.ok);
    H.ok(G.getState().candidateSelection.cardIds.includes('r1'));
    const confirmed = G.commitCandidate('r1', 'p1');
  ...
  H.test('candidate cardIds object from Firebase is normalized to an array', () => {
    const state = baseState();
    state.currentClue.submittedAt = '2026-09-10T00:00:00.000Z';
    state.candidateSelection = {
      team: 'red',
    ...
    G.replaceState(state, false);
    H.deepEqual(G.getState().candidateSelection.cardIds, ['r1', 'r2']);
  });

  H.test('candidate selection is included in public snapshot for spymaster/team visibility', () => {
    G.__test.replaceState(baseState());
    H.ok(G.toggleCandidate('r1', 'p1').ok);
    const publicState = G.getPublicSnapshot();
    H.ok(publicState.candidateSelection.cardIds.includes('r1'));
  });
