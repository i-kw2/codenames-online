...
// V18 public game-log actor metadata / بيانات منفذ الحدث في سجل اللعبة العام
(function () {
  'use strict';
  const H = window.TestHarness;
  const G = window.Codenames.Game;

  function v18State(status) {
    const state = G.__test.blankState({ uiLanguage: 'en', cardLanguage: 'both', strictClues: true, expertRules: true });
    Object.assign(state, {
      status: status || 'clue',
      currentTeam: 'blue',
      ...
      players: [
        { id: 'spy-blue', name: 'Blue Spy', team: 'blue', role: 'spymaster' },
        { id: 'op-blue', name: 'Blue Op', team: 'blue', role: 'operative' },
        { id: 'spy-red', name: 'Red Spy', team: 'red', role: 'spymaster' },
        { id: 'op-red', name: 'Red Op', team: 'red', role: 'operative' }
      ],
      board: [
        { id: 'b1', wordId: 'b1', ar: 'أزرق', en: 'BlueWord', role: 'blue', revealed: false },
        { id: 'b2', wordId: 'b2', ar: 'أزرق2', en: 'BlueTwo', role: 'blue', revealed: false },
        { id: 'r1', wordId: 'r1', ar: 'أحمر', en: 'RedWord', role: 'red', revealed: false },
        { id: 'r2', wordId: 'r2', ar: 'أحمر2', en: 'RedTwo', role: 'red', revealed: false },
        { id: 'n1', wordId: 'n1', ar: 'محايد', en: 'Neutral', role: 'neutral', revealed: false },
        { id: 'a1', wordId: 'a1', ar: 'قاتل', en: 'Assassin', role: 'assassin', revealed: false }
      ]
  ...
  H.test('V18 clue log keeps the submitting spymaster id and name', () => {
    G.__test.replaceState(v18State('clue'));
    const result = G.submitClue({ text: 'Ocean', mode: 'number', number: 1, actorId: 'spy-blue', actorName: 'Blue Spy' });
    H.ok(result.ok);
    const entry = G.getState().logs.find((item) => item.type === 'CLUE_SUBMITTED');
    H.ok(entry);
    H.equal(entry.payload.actorId, 'spy-blue');
    H.equal(entry.payload.actorName, 'Blue Spy');
    H.equal(entry.payload.team, 'blue');
    H.equal(entry.payload.text, 'Ocean');
  });

  H.test('V18 guess log keeps the operative id and name', () => {
    const state = v18State('guessing');
    state.currentClue = { text: 'Ocean', mode: 'number', number: 1, submittedByTeam: 'blue' };
    state.maxGuesses = 2;
    G.__test.replaceState(state);
    const result = G.handleGuess('b1', { actorId: 'op-blue', actorName: 'Blue Op' });
    H.ok(result.ok);
    const entry = G.getState().logs.find((item) => item.type === 'CARD_REVEALED');
    H.ok(entry);
    H.equal(entry.payload.actorId, 'op-blue');
    H.equal(entry.payload.actorName, 'Blue Op');
    H.equal(entry.payload.guessingTeam, 'blue');
    H.equal(entry.payload.en, 'BlueWord');
  });

  H.test('V18 public snapshot publishes game log but still hides unrevealed secret roles', () => {
    const state = v18State('guessing');
    state.currentClue = { text: 'Ocean', mode: 'number', number: 1, submittedByTeam: 'blue' };
    state.maxGuesses = 2;
    state.logs.push({ timestamp: new Date().toISOString(), type: 'CLUE_SUBMITTED', level: 'info', payload: { team: 'blue', text: 'Ocean', mode: 'number', number: 1, actorId: 'spy-blue', actorName: 'Blue Spy' } });
    G.__test.replaceState(state);
    const publicState = G.getPublicSnapshot();
    H.equal(publicState.logs.length, 1);
    H.equal(publicState.logs[0].payload.actorName, 'Blue Spy');
    H.equal(publicState.board.find((card) => card.id === 'a1').role, null);
    H.equal(publicState.board.find((card) => card.id === 'b1').role, null);
  });
})();
