...
async function randomizeTeams() {
  ...
  const players = Object.values(runtime.players).filter(Boolean);
  if (players.length < 4) {
    setMessage(dom.lobbyValidationMessage, 'Need at least 4 players in the room.');
...
async function setSelfAssignment(team, role) {
  if (!runtime.inRoom || runtime.roomStatus !== 'lobby' || !runtime.player) return { ok: false, code: 'NOT_IN_LOBBY' };
  if (!runtime.isHost && runtime.roomSettings.teamsLocked) return { ok: false, code: 'TEAMS_LOCKED', message: 'Teams are locked by the room admin.' };
  let nextTeam = team;
  let nextRole = role;
