...
ترسل الرابط لربعك. عند فتح رابط الدعوة تظهر نافذة **Welcome to Codenames**؛ يكتب الزائر Nickname ويضغط **ENTER GAME** ثم يدخل الغرفة كـSpectator. يستطيع الدخول قبل بدء المباراة أو أثناء مباراة جارية، ثم يختار فريقه ودوره. أنت الـHost وتتحكم ببدء اللعبة وإعداداتها.
...
هذه القواعد هي التي تمنع اللاعب العادي من قراءة Secret Key وتمنع اللاعبين من تعديل Game State مباشرة. في V16 تسمح القواعد لزائر موثّق Anonymous أن ينشئ سجل نفسه فقط كـSpectator سواء كانت الغرفة `lobby` أو `playing`، ثم يغيّر فريقه/دوره أثناء المباراة بدون إعطائه صلاحيات Host. الـHost هو الذي يكتب حالة اللعبة العامة، والـSpymasters فقط (بالإضافة إلى الـHost لأسباب المعالجة) يمكنهم قراءة مسار الأدوار السري.

> **مهم جدًا بعد الترقية إلى V16:** لا يكفي رفع ملفات الموقع. يجب نسخ `firebase-rules.json` الجديد إلى Realtime Database → Rules ثم الضغط على **Publish**. القواعد القديمة هي أحد الأسباب المباشرة لرسالة `permission_denied` عند فتح Invite Link.
...
5. الصق Invite Link.
6. يجب أن تظهر نافذة `Welcome to Codenames` وفيها `To enter the room, choose a nickname.`
7. اكتب اسم لاعب آخر واضغط **ENTER GAME**.
8. تأكد أنه ظهر أولًا داخل `Spectators`.
9. ابدأ المباراة من نافذة الـHost، ثم افتح Invite Link في نافذة Incognito جديدة مرة أخرى. يجب أن ينجح الدخول أيضًا أثناء المباراة، وبعدها يمكن للاعب اختيار الفريق/الدور من **Settings → Player**.
...
## V17 invite troubleshooting

If a friend sees the Welcome dialog but `ENTER GAME` returns a permission error:

1. Firebase Console → Authentication → Sign-in method → enable **Anonymous**.
2. Realtime Database → Rules → replace the deployed rules with this project's `firebase-rules.json` and press **Publish**.
3. Firebase Console → App Check → Realtime Database. If enforcement is ON, either configure Web App Check (reCAPTCHA provider) in the site code or turn enforcement OFF while testing.
4. Create a **new room after publishing the V17 rules**. New V17 rooms contain an `entry.rulesVersion = 17` marker used for diagnostics.
5. Test the invite link in a private/incognito window or a second browser/device.

V17 has a compatibility fallback for V13 rules while the room is still in `lobby`, but joining after a match has started requires the V17 server rules because old rules reject new players while status is `playing`.
