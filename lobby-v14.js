(function (global) {
  ...
  function validateLobby() {
    const active = players().filter((p) => !isSpectator(p) && ['red','blue'].includes(p.team));
    ...
    const errors = [];
    if (active.length < 4) errors.push(lang()==='ar' ? 'تحتاج 4 لاعبين نشطين على الأقل.' : 'Need at least 4 active players.');
  ...
  function translateSecondaryUi() {
    ...
    if(refs.rulesDialogV14){ const title=refs.rulesDialogV14.querySelector('h2'); if(title)title.textContent=q('CODENAMES — CLASSIC RULES','CODENAMES — قواعد الكلاسيك'); const sections=refs.rulesDialogV14.querySelectorAll('.rules-v14-content section'); const rr=ar?[['الفرق والأدوار','فريقان يتنافسان. لكل فريق قائد تجسس واحد ولاعب أو أكثر. قادة التجسس يعرفون الهوية السرية للبطاقات الـ25.'],['التلميحات','يعطي قائد التجسس النشط كلمة واحدة ورقماً واحداً. الرقم يحدد عدد كلمات اللوحة المرتبطة بالتلميح.'],['التخمين','للتلميح ذي الرقم n يستطيع الفريق إجراء حتى n + 1 تخمينات ما دامت الإجابات صحيحة، ويمكنه التوقف بعد تخمين واحد على الأقل.'],['البطاقات الخاطئة','البطاقة المحايدة تنهي الدور. بطاقة الخصم تُكشف لصالحه وتنهي الدور. القاتل ينهي اللعبة فوراً ويخسر الفريق الذي اختاره.'],['الفوز','يفوز الفريق فور كشف جميع عملائه، حتى لو كشف الخصم آخر عميل له بالخطأ.'],['قانونية التلميح','يجب أن يرتبط التلميح بالمعنى، لا بموقع البطاقات أو حيل التهجئة، ولا يجوز أن يكون كلمة ظاهرة على اللوحة أو صيغة محظورة منها.']]:[['Teams & roles','Two teams compete. Each team has one Spymaster and one or more Operatives. The Spymasters know the secret identities of the 25 cards.'],['Clues','The active Spymaster gives one word and one number. The number indicates how many board words are connected to that clue.'],['Guessing','For a clue numbered n, the team may make up to n + 1 guesses while its guesses remain correct. A team may stop early after making at least one guess.'],['Wrong cards','A Neutral card ends the turn. An opponent card is revealed for the opponent and ends the turn. The Assassin ends the game immediately and the guessing team loses.'],['Winning','A team wins immediately when all of its agents have been revealed, even if the opponent reveals the last one by mistake.'],['Clue legality','Clues must relate to meaning, not board position or spelling tricks, and cannot be a visible board word or a prohibited form of one.']]; sections.forEach((s,i)=>{if(rr[i]){const h=s.querySelector('h3'),p=s.querySelector('p');if(h)h.textContent=rr[i][0];if(p)p.textContent=rr[i][1];}});}
    if(refs.newsDialogV14){const h=refs.newsDialogV14.querySelector('h2'),p=refs.newsDialogV14.querySelector('p'); if(h)h.textContent=q('News','الأخبار'); if(p)p.textContent=q('V14 introduces the redesigned room lobby, Spectators thumbnails, team role panels, Settings tabs, contextual Quick Guide, team locking, and accessibility preferences while keeping the V13 game engine intact.','يقدم V14 تصميم Lobby الجديد، صور المشاهدين المصغرة، لوحات أدوار الفرق، تبويبات الإعدادات، الدليل السريع السياقي، قفل الفرق، وخيارات إمكانية الوصول مع الحفاظ على محرك V13.');}
  }
  ...
  function openSettings(tab) {
    ...
    renderSettings();
    if (refs.roomSettingsDialog && !refs.roomSettingsDialog.open) refs.roomSettingsDialog.showModal();
  }
  function closeSettings(){ if(refs.roomSettingsDialog && refs.roomSettingsDialog.open) refs.roomSettingsDialog.close(); }
  function openRules(){ if(refs.rulesDialogV14 && !refs.rulesDialogV14.open) refs.rulesDialogV14.showModal(); }
  ...
  function renderSettings() {
    ...
    document.querySelectorAll('[data-assign-team][data-assign-role]').forEach((b)=>{
      ...
      b.disabled=!lobbyEditable || (!context.isHost && Boolean(room.teamsLocked)) || spyTaken;
    });
    refs.prefUiLanguage.value=lang();
    refs.prefMusicVolume.value=safeGet('cn-v14-music-volume','0');
    refs.prefEffectsVolume.value=String(Math.round(Number(safeGet('cn-sound-volume','0.55'))*100));
    refs.prefAlwaysGuess.checked=boolPref('cn-v14-always-guess',false); refs.prefHideRoomUrl.checked=boolPref('cn-v14-hide-room-url',false);
    refs.a11yColorblind.checked=boolPref('cn-v14-colorblind',false); refs.a11ySeasonal.checked=boolPref('cn-v14-disable-seasonal',false);
    refs.a11yStaticBackground.checked=boolPref('cn-v14-static-bg',false); refs.a11yDisableAnimations.checked=boolPref('cn-v14-disable-animations',false);
    const theme=safeGet('cn-v14-theme','system'); document.querySelectorAll('[data-theme-choice]').forEach((b)=>b.classList.toggle('active',b.dataset.themeChoice===theme));
    const size=safeGet('cn-v14-text-size','small'); document.querySelectorAll('[data-text-size]').forEach((b)=>b.classList.toggle('active',b.dataset.textSize===size));
    const motion=safeGet('cn-v14-motion-mode','system'); document.querySelectorAll('[data-motion-mode]').forEach((b)=>b.classList.toggle('active',b.dataset.motionMode===motion));
  ...
  function guideSeen(){return boolPref('cn-v14-guide-setup-seen',false);}
  ...
  function maybeAutoGuide(){ if(!context.inRoom||context.roomStatus!=='lobby'||!refs.quickGuidePopover)return; const auto=boolPref('cn-v14-guide-auto',false); refs.quickGuideAutoShow.checked=auto; if(auto&&!guideSeen()&&refs.quickGuidePopover.hidden)openGuide(); }

  function applyPreferences() {
    const theme=safeGet('cn-v14-theme','system');
    let resolved=theme;
    if(theme==='system') resolved=global.matchMedia&&global.matchMedia('(prefers-color-scheme: dark)').matches?'dark':'light';
    document.documentElement.dataset.theme=resolved;
    document.documentElement.dataset.textSize=safeGet('cn-v14-text-size','small');
    document.body.classList.toggle('always-show-guess',boolPref('cn-v14-always-guess',false));
    document.body.classList.toggle('colorblind-symbols',boolPref('cn-v14-colorblind',false));
    document.body.classList.toggle('static-background',boolPref('cn-v14-static-bg',false));
    const mode=safeGet('cn-v14-motion-mode','system');
    const systemReduce=global.matchMedia&&global.matchMedia('(prefers-reduced-motion: reduce)').matches;
    const disable=mode==='system'?systemReduce:boolPref('cn-v14-disable-animations',false);
  ...
  function bind() {
    ...
    refs.roomNewsBtn&&refs.roomNewsBtn.addEventListener('click',()=>refs.newsDialogV14.showModal());
    ...
    refs.quickGuideGotItBtn&&refs.quickGuideGotItBtn.addEventListener('click',()=>{safeSet('cn-v14-guide-setup-seen','1');closeGuide();});
    refs.quickGuideRulesBtn&&refs.quickGuideRulesBtn.addEventListener('click',()=>{closeGuide();openRules();});
    refs.quickGuideAutoShow&&refs.quickGuideAutoShow.addEventListener('change',()=>safeSet('cn-v14-guide-auto',refs.quickGuideAutoShow.checked?'1':'0'));
    refs.roomSettingsCloseBtn&&refs.roomSettingsCloseBtn.addEventListener('click',closeSettings);
    refs.rulesCloseBtnV14&&refs.rulesCloseBtnV14.addEventListener('click',()=>refs.rulesDialogV14.close()); refs.newsCloseBtnV14&&refs.newsCloseBtnV14.addEventListener('click',()=>refs.newsDialogV14.close());
    ...
    document.querySelectorAll('[data-theme-choice]').forEach((b)=>b.addEventListener('click',()=>{safeSet('cn-v14-theme',b.dataset.themeChoice);applyPreferences();renderSettings();}));
    refs.prefMusicVolume&&refs.prefMusicVolume.addEventListener('input',()=>safeSet('cn-v14-music-volume',refs.prefMusicVolume.value));
    ...
    refs.prefAlwaysGuess&&refs.prefAlwaysGuess.addEventListener('change',()=>{safeSet('cn-v14-always-guess',refs.prefAlwaysGuess.checked?'1':'0');applyPreferences();});
    refs.prefHideRoomUrl&&refs.prefHideRoomUrl.addEventListener('change',()=>{safeSet('cn-v14-hide-room-url',refs.prefHideRoomUrl.checked?'1':'0'); const a=onlineApi(); if(a&&a.applyRoomUrlPreference)a.applyRoomUrlPreference();});
    refs.prefShowHintsBtn&&refs.prefShowHintsBtn.addEventListener('click',()=>{safeSet('cn-v14-guide-setup-seen','0');closeSettings();openGuide();});
    document.querySelectorAll('[data-text-size]').forEach((b)=>b.addEventListener('click',()=>{safeSet('cn-v14-text-size',b.dataset.textSize);applyPreferences();renderSettings();}));
    refs.a11yColorblind&&refs.a11yColorblind.addEventListener('change',()=>{safeSet('cn-v14-colorblind',refs.a11yColorblind.checked?'1':'0');applyPreferences();});
    refs.a11ySeasonal&&refs.a11ySeasonal.addEventListener('change',()=>safeSet('cn-v14-disable-seasonal',refs.a11ySeasonal.checked?'1':'0'));
    document.querySelectorAll('[data-motion-mode]').forEach((b)=>b.addEventListener('click',()=>{safeSet('cn-v14-motion-mode',b.dataset.motionMode);applyPreferences();renderSettings();}));
    [refs.a11yStaticBackground,refs.a11yDisableAnimations].filter(Boolean).forEach((el)=>el.addEventListener('change',()=>{safeSet('cn-v14-motion-mode','manual');safeSet('cn-v14-static-bg',refs.a11yStaticBackground.checked?'1':'0');safeSet('cn-v14-disable-animations',refs.a11yDisableAnimations.checked?'1':'0');applyPreferences();renderSettings();}));
  ...
  function init() {
    ...
    refs.quickGuideAutoShow.checked=boolPref('cn-v14-guide-auto',false);
