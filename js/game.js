(function (global) {
  ...
  // V11 FIX: use the untouched source PNG artwork from the stable assets/cards path and tint it only at display time.
  // V11: نستخدم صور PNG الأصلية دون تعديل، ويُطبق اللون فقط وقت العرض.
  const REVEAL_CARD_IMAGES = Object.freeze([
    'assets/cards/001_NN_Man.png', 'assets/cards/002_NN_Man.png', 'assets/cards/003_NN_Man.png', 'assets/cards/004_NN_Man.png', 'assets/cards/005_NN_Man.png',
    'assets/cards/006_NN_Woman.png', 'assets/cards/007_NN_Woman.png', 'assets/cards/008_NN_Woman.png', 'assets/cards/009_NN_Woman.png', 'assets/cards/010_NN_Woman.png',
    'assets/cards/011_NN_Animal.png', 'assets/cards/012_NN_Animal.png', 'assets/cards/013_NN_Animal.png', 'assets/cards/014_NN_Animal.png', 'assets/cards/015_NN_Animal.png',
    'assets/cards/016_NN_Kids.png', 'assets/cards/017_NN_Kids.png', 'assets/cards/018_NN_Kids.png', 'assets/cards/019_NN_Kids.png', 'assets/cards/020_NN_Kids.png',
    'assets/cards/021_NN_Maid.png', 'assets/cards/022_NN_Maid.png', 'assets/cards/023_NN_Maid.png', 'assets/cards/024_NN_Maid.png', 'assets/cards/025_NN_Maid.png'
  ]);
  ...
  function submitClue(input) {
    ...
    logEvent('CLUE_SUBMITTED', { team: gameState.currentTeam, text: validation.clue, mode, number: gameState.currentClue.number, actorId: clue.actorId || null, actorName: clue.actorName || null });
    notify();
   ...
   */
  function revealCard(cardId, guessingTeam, source, actor) {
    ...
    logEvent('CARD_REVEALED', { cardId: card.id, wordId: card.wordId, ar: card.ar, en: card.en, role: card.role, outcome, guessingTeam, source: source || 'guess', actorId: actor && actor.actorId ? actor.actorId : null, actorName: actor && actor.actorName ? actor.actorName : null, clue: gameState.currentClue ? U.deepClone(gameState.currentClue) : null, remaining: U.deepClone(gameState.remaining) });
  ...
  function endTurn(reason, options) {
    ...
    gameState.status = 'clue';
    logEvent('TURN_ENDED', { team: endingTeam, nextTeam, reason: reason || 'VOLUNTARY', clue: endingClue, guessesMade: endingGuesses, actorId: opts.actorId || null, actorName: opts.actorName || null });
    logEvent('TURN_STARTED', { team: nextTeam, previousTeam: endingTeam });
    notify();
    return { ok: true, nextTeam };
  }

  function handleGuess(cardId, actor) {
    ...
    try {
      ...
      const result = revealCard(cardId, guessingTeam, 'guess', actor);
      if (!result.ok || gameState.status === 'ended') { notify(); return result; }

      if (result.role === 'neutral') {
        return endTurn('NEUTRAL_GUESS', { force: true, actorId: actor && actor.actorId, actorName: actor && actor.actorName });
      }
      if (result.role !== guessingTeam) {
        return endTurn('OPPONENT_AGENT', { force: true, actorId: actor && actor.actorId, actorName: actor && actor.actorName });
      }
      if (Number.isFinite(gameState.maxGuesses) && gameState.guessesMade >= gameState.maxGuesses) {
        return endTurn('GUESS_LIMIT_REACHED', { force: true, actorId: actor && actor.actorId, actorName: actor && actor.actorName });
      }
