(function (global) {
  ...
  function blankState(settings) {
    return {
      ...
      errorLogs: [],
      clueChallenge: null,
      candidateSelection: { team: null, cardIds: [], clueSubmittedAt: null, updatedAt: null, updatedBy: null },
  ...
  /**
   * Normalizes candidate-selection data after Firebase RTDB round-trips.
   * Firebase may remove empty arrays ([]) from a stored object, so cardIds
   * can arrive as missing/null even though the selection context is valid.
   *
   * يطبّع بيانات البطاقات المحتملة بعد مزامنة Firebase. قد يحذف Firebase
   * المصفوفة الفارغة، لذلك نعيد cardIds دائماً إلى مصفوفة آمنة.
   */
  function normalizeCandidateSelection(value) {
    const source = value && typeof value === 'object' ? value : {};
    let ids = [];
    if (Array.isArray(source.cardIds)) ids = source.cardIds;
    else if (source.cardIds && typeof source.cardIds === 'object') ids = Object.values(source.cardIds);
    ids = ids.filter((id, index, list) => typeof id === 'string' && id && list.indexOf(id) === index);
    return {
      team: source.team === 'red' || source.team === 'blue' ? source.team : null,
      cardIds: ids,
      clueSubmittedAt: source.clueSubmittedAt || null,
  ...
  function replaceState(nextState, shouldNotify) {
    ...
    gameState.candidateSelection = normalizeCandidateSelection(gameState.candidateSelection);
  ...
  function getPublicSnapshot() {
    ...
    snapshot.candidateSelection = normalizeCandidateSelection(snapshot.candidateSelection);
  ...
  function startTurn(team) {
    ...
    gameState.candidateSelection = { team: null, cardIds: [], clueSubmittedAt: null, updatedAt: null, updatedBy: null };
  ...
  function submitClue(input) {
    ...
    gameState.candidateSelection = { team: gameState.currentTeam, cardIds: [], clueSubmittedAt: gameState.currentClue.submittedAt, updatedAt: nowISO(), updatedBy: null };
  ...
  function endGame(winnerTeam, reason) {
    ...
    gameState.currentClue = null;
    gameState.candidateSelection = { team: null, cardIds: [], clueSubmittedAt: null, updatedAt: nowISO(), updatedBy: null };
    gameState.invalidCluePenalty = null;
  ...
  function ensureCandidateSelectionContext() {
    const submittedAt = gameState.currentClue && gameState.currentClue.submittedAt ? gameState.currentClue.submittedAt : null;
    gameState.candidateSelection = normalizeCandidateSelection(gameState.candidateSelection);
    if (gameState.candidateSelection.team !== gameState.currentTeam || gameState.candidateSelection.clueSubmittedAt !== submittedAt) {
      gameState.candidateSelection = { team: gameState.currentTeam, cardIds: [], clueSubmittedAt: submittedAt, updatedAt: nowISO(), updatedBy: null };
    }
    return gameState.candidateSelection;
  }

  function toggleCandidate(cardId, actorId) {
    ...
    const selection = ensureCandidateSelectionContext();
    const index = selection.cardIds.indexOf(cardId);
    const added = index === -1;
    if (added) selection.cardIds.push(cardId); else selection.cardIds.splice(index, 1);
    selection.updatedAt = nowISO();
    selection.updatedBy = actorId || null;
    logEvent(added ? 'CANDIDATE_ADDED' : 'CANDIDATE_REMOVED', { team: gameState.currentTeam, cardId, wordId: card.wordId, ar: card.ar, en: card.en, playerId: actorId || null, clue: gameState.currentClue ? U.deepClone(gameState.currentClue) : null });
    notify();
    return { ok: true, selected: added, cardIds: selection.cardIds.slice() };
  }

  function clearCandidates(actorId, reason) {
    if (gameState.status !== 'guessing') return { ok: false, code: 'INVALID_PHASE' };
    const selection = ensureCandidateSelectionContext();
    selection.cardIds = [];
    selection.updatedAt = nowISO();
    selection.updatedBy = actorId || null;
    logEvent('CANDIDATES_CLEARED', { team: gameState.currentTeam, playerId: actorId || null, reason: reason || 'MANUAL', clue: gameState.currentClue ? U.deepClone(gameState.currentClue) : null });
    notify();
    return { ok: true };
  }

  function commitCandidate(cardId, actorId) {
    if (transitionLock) return { ok: false, code: 'BUSY' };
    if (gameState.status !== 'guessing') return { ok: false, code: 'INVALID_PHASE' };
    const selection = ensureCandidateSelectionContext();
    if (!selection.cardIds.includes(cardId)) return { ok: false, code: 'CANDIDATE_NOT_SELECTED' };
    const card = gameState.board.find((item) => item.id === cardId);
    if (!card || card.revealed) return { ok: false, code: card ? 'ALREADY_REVEALED' : 'CARD_NOT_FOUND' };
    const previous = U.deepClone(selection);
    selection.cardIds = [];
    selection.updatedAt = nowISO();
    selection.updatedBy = actorId || null;
    const result = handleGuess(cardId);
    if (!result.ok) {
      gameState.candidateSelection = previous;
      notify();
      return result;
    }
    logEvent('GUESS_CONFIRMED', { team: previous.team, cardId, wordId: card.wordId, ar: card.ar, en: card.en, playerId: actorId || null, clue: gameState.currentClue ? U.deepClone(gameState.currentClue) : null });
    return result;
  }
