(function (global) {
  ...
  function cacheRefs() {
    [
      ...
      'gameInstruction','blueOperativeRoster','blueSpymasterRoster','redOperativeRoster','redSpymasterRoster','blueOperativesPanel','blueSpymastersPanel','redOperativesPanel','redSpymastersPanel',
      'currentClueValue','currentClueCount','operativeClueBar','waitingClueBar','waitingClueText','guessCounter','expertHint','spymasterControls','clueForm','clueTextInput','clueModeSelect','clueNumberField','clueNumberInput','clueCountPickerBtn','clueNumberPicker','clueSubmitBtn','clueValidationMessage',
      ...
      'gameLogPanel','gameLogStream','gameLogCollapseBtn','gameLogBtn','errorLogBtn','resetGameBtn','newGameBtn','privacyDialog','holdRevealBtn','holdProgress','challengeDialog','challengeClueText','allowClueBtn','rejectClueBtn',
  ...
  function gameAvatarSrc(player) {
    return `assets/cards/${GAME_AVATARS[gameAvatarId(player) - 1]}`;
  }

  function makeGamePlayer(player, compact) {
    const item = document.createElement('div');
    item.className = `game-player-v18${compact ? ' compact' : ''}${player && player.online === false ? ' offline' : ''}`;
    if (player && player.isHost) {
      const crown = document.createElement('span');
      crown.className = 'game-player-crown-v18';
      crown.textContent = '👑';
      crown.setAttribute('aria-hidden', 'true');
      item.appendChild(crown);
    }
    const img = document.createElement('img');
    img.className = 'game-player-avatar-v18';
    img.src = gameAvatarSrc(player || {});
    img.alt = '';
    img.loading = 'lazy';
    const name = document.createElement('span');
    name.className = 'game-player-name-v18';
    name.textContent = player && player.name ? player.name : 'Player';
  ...
  function setOnlineContext(next) {
    ...
    // Online devices know their own role, so spymasters see the secret key immediately.
    // في اللعب الأونلاين الجهاز يعرف دور اللاعب، لذلك يظهر المفتاح للقائد مباشرة بدون زر أو ضغط مطول.
    const player = myOnlinePlayer();
    viewMode = player && player.role === 'spymaster' && onlineContext.secretReady ? 'spymaster' : 'operative';
    render(G.getState());
  }
  ...
  async function requestOnlineAction(type, payload) {
    ...
    // Do not even send host-only game-management actions from a non-host client.
    // لا نرسل أوامر إعادة الجولة/اللعبة الجديدة من جهاز غير المضيف أصلاً.
    if ((type === 'resetRound' || type === 'returnLobby') && !onlineContext.isHost) return { ok: false, code: 'HOST_ONLY' };
  ...
  function renderWordText(card, state, button) {
    if (card.revealed) {
      if (card.revealImage) {
        ...
        image.className = 'card-reveal-image';
        // Resolve both the current V11 path and the earlier V11 /original/ path.
        // ندعم المسار الحالي ومسار V11 السابق حتى لا تنكسر الغرف المحفوظة قبل الإصلاح.
        const originalSrc = String(card.revealImage || '');
        const fileName = originalSrc.split('/').pop();
        const fallbackSrc = fileName ? `assets/cards/${fileName}` : '';
        image.alt = '';
        ...
        image.addEventListener('error', () => {
          ...
          // Hide the browser's broken-image icon if the deployment is missing the asset.
          // نخفي رمز الصورة المكسورة إذا كانت ملفات الصور غير مرفوعة على الاستضافة.
          image.hidden = true;
          button.classList.add('reveal-image-missing');
        }, { once: false });
        image.src = originalSrc.replace('assets/cards/original/', 'assets/cards/');
        button.appendChild(image);
      } else {
        // Backward compatibility for a legacy room snapshot without image assignments.
        // توافق رجعي مع جولة قديمة لا تحتوي على ربط صور.
  ...
  function renderClue(state) {
    if (!state.currentClue) {
      ...
      if (refs.currentClueCount) refs.currentClueCount.textContent = '—';
    ...
    refs.currentClueValue.textContent = clue.text;
    if (refs.currentClueCount) refs.currentClueCount.textContent = String(suffix);
  ...
  function renderRoster(state) {
    const roleBuckets = {
      blueOperativeRoster: [], blueSpymasterRoster: [],
      redOperativeRoster: [], redSpymasterRoster: []
    };
    (state.players || []).forEach((player) => {
      if (!player || player.team === 'spectator' || player.role === 'spectator') return;
      const key = `${player.team}${player.role === 'spymaster' ? 'SpymasterRoster' : 'OperativeRoster'}`;
      if (roleBuckets[key]) roleBuckets[key].push(player);
    });

    Object.keys(roleBuckets).forEach((key) => {
      const node = refs[key];
      if (!node) return;
      node.replaceChildren();
      roleBuckets[key].forEach((player) => node.appendChild(makeGamePlayer(player, false)));
      if (!node.children.length) {
        const empty = document.createElement('span');
        empty.className = 'game-role-empty-v18';
        empty.textContent = '—';
        node.appendChild(empty);
      }
    });

    // Compatibility roster nodes retained for older UI/debug hooks.
    if (refs.redRoster) refs.redRoster.replaceChildren();
    if (refs.blueRoster) refs.blueRoster.replaceChildren();
    if (refs.spectatorRoster) refs.spectatorRoster.replaceChildren();
    (state.players || []).forEach((player) => {
      const key = player.team === 'spectator' || player.role === 'spectator' ? 'spectator' : player.team;
      const host = key === 'red' ? refs.redRoster : key === 'blue' ? refs.blueRoster : refs.spectatorRoster;
      if (!host) return;
      const item = document.createElement('span');
      item.textContent = `${player.name} · ${roleLabel(player.role)}`;
  ...
  function findTeamPlayer(state, team, role) {
    return (state.players || []).find((player) => player && player.team === team && player.role === role) || null;
  }

  function findLogPlayer(state, payload, team, preferredRole) {
    const p = payload || {};
    if (p.actorId) {
      const exact = (state.players || []).find((player) => player && (player.id === p.actorId || player.uid === p.actorId));
      if (exact) return exact;
    }
    if (p.actorName) {
      const byName = (state.players || []).find((player) => player && player.name === p.actorName);
      if (byName) return byName;
    }
    return findTeamPlayer(state, team, preferredRole) || { id: `fallback-${team}-${preferredRole}`, name: p.actorName || (preferredRole === 'spymaster' ? 'Spymaster' : 'Operative'), team, role: preferredRole };
  }

  function gameInstructionText(state, player) {
    if (state.status === 'ended') return `${teamLabel(state.winner).toUpperCase()} TEAM WINS`;
    if (state.status === 'penalty') return I.getLanguage() === 'ar' ? 'حل عقوبة التلميح' : 'RESOLVE THE CLUE PENALTY';
    const currentSpy = findTeamPlayer(state, state.currentTeam, 'spymaster');
    if (state.status === 'clue') {
      if (!isOnline() && viewMode === 'spymaster') return 'GIVE YOUR OPERATIVES A CLUE';
      if (player && player.team === state.currentTeam && player.role === 'spymaster') return 'GIVE YOUR OPERATIVES A CLUE';
      return currentSpy ? `WAIT FOR ${currentSpy.name} TO GIVE YOU A CLUE` : `${String(state.currentTeam || '').toUpperCase()} TEAM NEEDS A SPYMASTER`;
    }
    if (state.status === 'guessing') {
      if (!isOnline() && viewMode === 'operative') return 'TAP ON CARDS YOU THINK MATCH THE CLUE';
      if (player && player.team === state.currentTeam && player.role === 'operative') return 'TAP ON CARDS YOU THINK MATCH THE CLUE';
      if (player && player.team === state.currentTeam && player.role === 'spymaster') return 'YOUR OPERATIVES ARE GUESSING';
      return `WAIT FOR ${teamLabel(state.currentTeam).toUpperCase()} TEAM TO GUESS`;
    }
    return phaseLabel(state.status).toUpperCase();
  }

  function clueSuffix(clue) {
    if (!clue) return '—';
    return clue.mode === 'number' ? String(clue.number) : clue.mode === 'zero' ? '0' : '∞';
  }

  function displayWordFromLogPayload(payload, state) {
    const p = payload || {};
    if (state.settings.cardLanguage === 'ar') return p.ar || p.en || p.wordId || '—';
    if (state.settings.cardLanguage === 'en') return p.en || p.ar || p.wordId || '—';
    const primary = I.getLanguage() === 'ar' ? p.ar : p.en;
    const secondary = I.getLanguage() === 'ar' ? p.en : p.ar;
    return primary && secondary ? `${primary} / ${secondary}` : (primary || secondary || p.wordId || '—');
  }

  function renderGameLog(state) {
    if (!refs.gameLogStream) return;
    const logs = Array.isArray(state.logs) ? state.logs : [];
    const groups = [];
    let current = null;
    logs.forEach((entry) => {
      const p = entry.payload || {};
      if (entry.type === 'CLUE_SUBMITTED') {
        current = {
          team: p.team,
          text: p.text,
          number: p.mode === 'unlimited' ? '∞' : (p.mode === 'zero' ? '0' : p.number),
          actor: findLogPlayer(state, p, p.team, 'spymaster'),
          guesses: [],
          ended: null
        };
        groups.push(current);
      ...
      if (entry.type === 'CARD_REVEALED' && (p.source || 'guess') === 'guess') {
        let target = current;
        if (!target || target.team !== p.guessingTeam) {
          target = groups.slice().reverse().find((g) => g.team === p.guessingTeam && !g.ended) || groups[groups.length - 1];
        }
        if (target) {
          target.guesses.push({
            player: findLogPlayer(state, p, p.guessingTeam, 'operative'),
            word: displayWordFromLogPayload(p, state),
            outcome: p.outcome || '',
            role: p.role || ''
          });
        }
      ...
      if (entry.type === 'TURN_ENDED' && current && current.team === p.team) current.ended = p.reason || 'TURN_ENDED';
    });

    const wasNearBottom = refs.gameLogStream.scrollHeight - refs.gameLogStream.scrollTop - refs.gameLogStream.clientHeight < 70;
    refs.gameLogStream.replaceChildren();
    if (!groups.length) {
      const empty = document.createElement('div');
      empty.className = 'game-log-empty-v18';
      empty.textContent = I.getLanguage() === 'ar' ? 'لا توجد أحداث بعد' : 'No events yet';
      refs.gameLogStream.appendChild(empty);
      return;
    }

    groups.slice(-24).forEach((group) => {
      const turn = document.createElement('section');
      turn.className = `game-log-turn-v18 team-${group.team}`;
      const header = document.createElement('div');
      header.className = 'game-log-clue-v18';
      const actor = makeGamePlayer(group.actor, true);
      const clue = document.createElement('strong');
      clue.className = 'game-log-clue-word-v18';
      clue.textContent = group.text || '—';
      const count = document.createElement('span');
      count.className = 'game-log-clue-count-v18';
      count.textContent = String(group.number == null ? '—' : group.number);
      header.append(actor, clue, count);
      turn.appendChild(header);
      group.guesses.forEach((guess) => {
        const row = document.createElement('div');
        row.className = `game-log-guess-v18 outcome-${guess.outcome || 'unknown'}`;
        const who = makeGamePlayer(guess.player, true);
        const word = document.createElement('span');
        word.className = 'game-log-guess-word-v18';
        word.textContent = guess.word;
        const mark = document.createElement('span');
        mark.className = 'game-log-guess-mark-v18';
        mark.textContent = guess.outcome === 'assassin' ? '☠' : guess.outcome === 'neutral' ? '◇' : guess.outcome === 'opponent' ? '↔' : '✓';
        row.append(who, word, mark);
        turn.appendChild(row);
      });
      refs.gameLogStream.appendChild(turn);
    });
    if (wasNearBottom || refs.gameLogStream.dataset.initialized !== '1') refs.gameLogStream.scrollTop = refs.gameLogStream.scrollHeight;
    refs.gameLogStream.dataset.initialized = '1';
  }

  function syncCluePickerFromInputs() {
    if (!refs.clueCountPickerBtn) return;
    const mode = refs.clueModeSelect.value;
    refs.clueCountPickerBtn.textContent = mode === 'zero' ? '0' : mode === 'unlimited' ? '∞' : String(Math.max(1, Math.min(9, Number(refs.clueNumberInput.value) || 1)));
  }

  function renderGame(state) {
    if (isOnline()) {
      const devicePlayer = myOnlinePlayer();
      const shouldSeeSecret = Boolean(devicePlayer && devicePlayer.role === 'spymaster' && onlineContext.secretReady);
      viewMode = shouldSeeSecret ? 'spymaster' : 'operative';
    }
    ...
    refs.secretWarning.hidden = viewMode !== 'spymaster' || isOnline();
    refs.secretWarning.textContent = I.t('rolesVisibleWarning');

    const player = myOnlinePlayer();
    if (refs.gameInstruction) refs.gameInstruction.textContent = gameInstructionText(state, player);

    renderClue(state);
    renderRoster(state);
    renderBoard(state);
    if (CN.Effects && typeof CN.Effects.requestInitialDeal === 'function') CN.Effects.requestInitialDeal(state);
    renderPenalty(state);
    renderGameLog(state);
    ...
    const showOperativeBar = state.status === 'guessing' && Boolean(state.currentClue);

    refs.spymasterControls.hidden = !canSubmitClue;
    if (refs.operativeClueBar) refs.operativeClueBar.hidden = !showOperativeBar;
    if (refs.waitingClueBar) refs.waitingClueBar.hidden = canSubmitClue || showOperativeBar || state.status === 'ended' || state.status === 'penalty';
    if (refs.waitingClueText && !refs.waitingClueBar.hidden) refs.waitingClueText.textContent = gameInstructionText(state, player);

    // Highlight the role that currently owns the action.
    ['blueOperativesPanel','blueSpymastersPanel','redOperativesPanel','redSpymastersPanel'].forEach((key) => {
      if (refs[key]) refs[key].classList.remove('active-turn');
    });
    if (state.status === 'clue' && state.currentTeam) {
      const panel = refs[`${state.currentTeam}SpymastersPanel`];
      if (panel) panel.classList.add('active-turn');
    } else if (state.status === 'guessing' && state.currentTeam) {
      const panel = refs[`${state.currentTeam}OperativesPanel`];
      if (panel) panel.classList.add('active-turn');
    }

    // The privacy/hold button is only useful for local shared-device play.
    refs.spymasterKeyBtn.hidden = isOnline() || viewMode === 'spymaster' || state.status === 'ended';
    refs.spymasterKeyBtn.disabled = false;
    refs.hideSpymasterBtn.hidden = isOnline() || viewMode !== 'spymaster';

    const challengeViewAllowed = isOnline() ? viewMode === 'spymaster' : viewMode === 'operative';
    refs.challengeClueBtn.hidden = !(state.status === 'guessing' && state.guessesMade === 0 && challengeViewAllowed && isOpposingSpy && state.clueChallenge && state.clueChallenge.status === 'open');
    refs.endTurnBtn.disabled = guessBusy || !(state.status === 'guessing' && state.guessesMade >= 1 && viewMode === 'operative' && isCurrentOperative);
    refs.endTurnBtn.hidden = !showOperativeBar;

    // Online game-management controls remain host-only in the hidden compatibility strip.
    const hideHostGameControls = isOnline() && !onlineContext.isHost;
    refs.resetGameBtn.hidden = hideHostGameControls;
    refs.newGameBtn.hidden = hideHostGameControls;
    refs.resetGameBtn.disabled = hideHostGameControls;
    refs.newGameBtn.disabled = hideHostGameControls;
    ...
    const expert = Boolean(state.settings.expertRules);
    Array.from(refs.clueModeSelect.options).forEach((option) => {
      option.disabled = option.value !== 'number' && !expert;
    });
    if (!expert && refs.clueModeSelect.value !== 'number') {
      refs.clueModeSelect.value = 'number';
      refs.clueNumberInput.value = Math.max(1, Math.min(9, Number(refs.clueNumberInput.value) || 1));
    }
    refs.clueNumberField.hidden = refs.clueModeSelect.value !== 'number';
    if (refs.clueNumberPicker) {
      refs.clueNumberPicker.querySelectorAll('[data-clue-pick="zero"],[data-clue-pick="unlimited"]').forEach((button) => { button.disabled = !expert; });
      refs.clueNumberPicker.querySelectorAll('[data-clue-pick]').forEach((button) => {
        const mode = button.dataset.cluePick;
        const selected = mode === refs.clueModeSelect.value && (mode !== 'number' || Number(button.dataset.value) === Number(refs.clueNumberInput.value));
        button.classList.toggle('selected', selected);
        button.setAttribute('aria-selected', selected ? 'true' : 'false');
      });
    }
    syncCluePickerFromInputs();

    if (refs.gameLogPanel) refs.gameLogPanel.classList.toggle('collapsed', gameLogCollapsed);
    if (refs.gameLogCollapseBtn) {
      refs.gameLogCollapseBtn.textContent = gameLogCollapsed ? '⌄' : '⌃';
      refs.gameLogCollapseBtn.setAttribute('aria-expanded', gameLogCollapsed ? 'false' : 'true');
    }
  }
  ...
  function render(state) {
    ...
    if (runtimeMode === 'menu') {
      ...
      document.body.classList.remove('game-v18-active');
      return;
    }
    if (runtimeMode === 'online') {
      ...
      document.body.classList.toggle('game-v18-active', showGame);
    ...
    document.body.classList.toggle('game-v18-active', !isSetup);
    if (isSetup) {
      renderPlayers();
  ...
  async function submitClue(event) {
    ...
    const localSpy = !isOnline() ? findTeamPlayer(stateBefore, stateBefore.currentTeam, 'spymaster') : null;
    const input = { text: refs.clueTextInput.value, mode, number: Number(refs.clueNumberInput.value), actorId: localSpy && localSpy.id, actorName: localSpy && localSpy.name };
    ...
    if (!isOnline()) viewMode = 'operative';
  ...
  async function handleBoardClick(event) {
    ...
    try {
      ...
      const localOperative = !isOnline() ? findTeamPlayer(before, before.currentTeam, 'operative') : null;
      const result = isOnline() ? await requestOnlineAction('guess', { cardId: card.id }) : G.handleGuess(card.id, { actorId: localOperative && localOperative.id, actorName: localOperative && localOperative.name });
      if (!result.ok) {
      ...
      else if (state.currentTeam === teamBefore) toast(I.t('guessedOwn'));
      // Turn-change feedback is handled by the centered animated overlay.
      // إشعار تبديل الدور يظهر في منتصف الشاشة عبر طبقة الأنميشن.
    } finally {
      guessBusy = false;
  ...
  async function endTurn() {
    const before = G.getState();
    const localOperative = !isOnline() ? findTeamPlayer(before, before.currentTeam, 'operative') : null;
    const result = isOnline() ? await requestOnlineAction('endTurn', {}) : G.endTurn('VOLUNTARY', { actorId: localOperative && localOperative.id, actorName: localOperative && localOperative.name });
    if (!result.ok) {
      toast(result.code === 'MUST_GUESS_FIRST' ? I.t('mustGuessFirst') : (result.code || I.t('wrongPhase')));
      return;
    }
    // The centered turn overlay announces the next team.
    // إشعار منتصف الشاشة يعلن الفريق التالي.
  }
  ...
  async function skipPenalty() {
    ...
    if (result.ok) { if (!isOnline()) viewMode = 'operative'; render(G.getState()); }
    else toast(result.code || I.t('wrongPhase'));
  }
  ...
  function toggleClueNumberPicker(forceOpen) {
    if (!refs.clueNumberPicker || !refs.clueCountPickerBtn) return;
    const open = typeof forceOpen === 'boolean' ? forceOpen : refs.clueNumberPicker.hidden;
    refs.clueNumberPicker.hidden = !open;
    refs.clueCountPickerBtn.setAttribute('aria-expanded', open ? 'true' : 'false');
  }

  function chooseClueNumber(event) {
    const button = event.target.closest('[data-clue-pick]');
    if (!button || button.disabled) return;
    const mode = button.dataset.cluePick;
    if ((mode === 'zero' || mode === 'unlimited') && !G.getState().settings.expertRules) {
      toast(I.t('expertRules'));
      return;
    }
    refs.clueModeSelect.value = mode;
    if (mode === 'number') refs.clueNumberInput.value = String(Math.max(1, Math.min(9, Number(button.dataset.value) || 1)));
    refs.clueNumberField.hidden = mode !== 'number';
    syncCluePickerFromInputs();
    toggleClueNumberPicker(false);
    render(G.getState());
  }

  function toggleGameLogCollapse() {
    gameLogCollapsed = !gameLogCollapsed;
    if (refs.gameLogPanel) refs.gameLogPanel.classList.toggle('collapsed', gameLogCollapsed);
    if (refs.gameLogCollapseBtn) {
      refs.gameLogCollapseBtn.textContent = gameLogCollapsed ? '⌄' : '⌃';
      refs.gameLogCollapseBtn.setAttribute('aria-expanded', gameLogCollapsed ? 'false' : 'true');
  ...
  function bindEvents() {
    ...
    if (refs.clueCountPickerBtn) refs.clueCountPickerBtn.addEventListener('click', () => toggleClueNumberPicker());
    if (refs.clueNumberPicker) refs.clueNumberPicker.addEventListener('click', chooseClueNumber);
    if (refs.gameLogCollapseBtn) refs.gameLogCollapseBtn.addEventListener('click', toggleGameLogCollapse);
    document.addEventListener('pointerdown', (event) => {
      if (!refs.clueNumberPicker || refs.clueNumberPicker.hidden) return;
      if (event.target.closest('#clueNumberPicker, #clueCountPickerBtn')) return;
      toggleClueNumberPicker(false);
    });
