  ...
  function renderWordText(card, state, button) {
    if (card.revealed && viewMode === 'operative') {
      if (card.revealImage) {
        ...
        image.src = card.revealImage;
        image.alt = '';
        image.decoding = 'async';
        image.draggable = false;
        button.appendChild(image);
      } else {
        // Backward compatibility for a legacy V9 room snapshot without image assignments.
        // توافق رجعي مع جولة V9 قديمة لا تحتوي على ربط صور.
