(function (global) {
  ...
  function renderBoard(state) {
    ...
    state.board.forEach((card) => {
      ...
      if (!card.revealed && candidateIds.has(card.id)) { button.classList.add('candidate-selected'); if (viewMode === 'spymaster') button.classList.add('candidate-spymaster'); button.setAttribute('aria-pressed', 'true'); }
  ...
  function syncCandidateContext(state) {
    ...
    if (candidateContextKey && candidateContextKey !== key) candidateConfirmBusy = false;
    candidateContextKey = key;
    candidateIds.clear();
    const selection = state.candidateSelection;
    if (state.status === 'guessing' && selection && selection.team === state.currentTeam && Array.isArray(selection.cardIds)) {
      selection.cardIds.forEach((id) => candidateIds.add(id));
    }
  }
  ...
  function renderCandidates(state) {
    const player = myOnlinePlayer();
    const canModify = state.status === 'guessing' && viewMode === 'operative' && canCurrentClientGuess(state) && !(state.clueChallenge && state.clueChallenge.status === 'pending');
    const canSpySee = state.status === 'guessing' && viewMode === 'spymaster' && (!isOnline() || Boolean(player && player.role === 'spymaster' && player.team === state.currentTeam));
    const show = canModify || canSpySee;
    refs.candidatePanel.hidden = !show;
    refs.clearCandidatesBtn.hidden = !canModify;
    refs.clearCandidatesBtn.disabled = !canModify || candidateConfirmBusy || candidateIds.size === 0;
    refs.candidateList.replaceChildren();
    if (!show) return;
    const cards = state.board.filter((card) => candidateIds.has(card.id) && !card.revealed);
    if (!cards.length) {
      const empty = document.createElement('p'); empty.className = 'small muted candidate-empty'; empty.textContent = canSpySee ? I.t('spymasterCandidateHelp') : I.t('candidateHelp'); refs.candidateList.appendChild(empty); return;
    }
    cards.forEach((card) => {
      ...
      if (canModify) {
        ...
        const badge = document.createElement('span'); badge.className = 'candidate-watch-badge'; badge.textContent = I.t('teamConsidering'); row.append(badge);
      }
      refs.candidateList.appendChild(row);
    });
  }
  ...
  function renderGame(state) {
    ...
    refs.endTurnBtn.disabled = !(state.status === 'guessing' && state.guessesMade >= 1 && viewMode === 'operative' && isCurrentOperative);
  ...
  async function handleBoardClick(event) {
    ...
    const result = isOnline() ? await requestOnlineAction('toggleCandidate', { cardId: card.id }) : G.toggleCandidate(card.id, 'local-operative');
  ...
  async function confirmCandidate(cardId) {
    ...
    try {
      const result = isOnline() ? await requestOnlineAction('commitCandidate', { cardId }) : G.commitCandidate(cardId, 'local-operative');
      if (!result.ok) {
  ...
  async function clearCandidates() {
    ...
    const result = isOnline() ? await requestOnlineAction('clearCandidates', {}) : G.clearCandidates('local-operative', 'MANUAL');
    if (!result.ok) toast(result.code || I.t('wrongPhase'));
  }
