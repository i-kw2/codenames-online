...
async function savePlayerProfile(name, avatarId) {
  ...
  if (!clean) return { ok: false, code: 'INVALID_NAME' };
  const id = Math.max(1, Math.min(25, Math.trunc(Number(avatarId) || 1)));
  // Commit profile fields together so every client sees one coherent saved profile.
...
async function processOneAction(actionId, action) {
  ...
  try {
    ...
    const roomManagementOnly = action.type === 'assignSelf';
    ...
    if (authz.ok) {
      ...
      if (result.ok && !['resetRound', 'returnLobby', 'assignSelf'].includes(action.type)) await broadcastHostState(false);
    }
