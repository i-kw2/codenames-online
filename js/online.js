...
const runtime = {
  ...
  joinCompatibilityMode: 'modern',
  roomRulesVersion: null,
  inRoom: false,
  isHost: false,
  roomStatus: null,
  players: {},
...
function playerTeamLabel(team) {
  ...
  if (team === 'spectator' || team === 'unassigned') return '👁️ Spectator';
  return '⚪ Unassigned';
}
...
function cleanupListeners() {
  ...
  runtime.playersSnapshotReady = false;
  runtime.activePlayerIds = new Set();
}
...
function resetRoomRuntime() {
  ...
  runtime.joinCompatibilityMode = 'modern';
  runtime.roomRulesVersion = null;
...
function livePlayersForGameState() {
  return Object.values(runtime.players || {}).filter(Boolean).map((player, index) => {
    const spectator = player.team === 'spectator' || player.team === 'unassigned' || player.role === 'spectator';
    return {
...
function attachRoomListeners() {
  ...
  runtime.unsubs.push(onValue(roomRef('players'), (snap) => {
    const nextPlayers = snap.val() || {};
    Object.keys(nextPlayers).forEach((uid) => { if (nextPlayers[uid]) nextPlayers[uid].uid = uid; });
    const nextActiveIds = new Set(Object.entries(nextPlayers).filter(([, player]) => player && player.online !== false).map(([uid]) => uid));
    if (runtime.playersSnapshotReady) {
      nextActiveIds.forEach((uid) => {
        if (uid !== runtime.uid && !runtime.activePlayerIds.has(uid) && CN && CN.Effects && typeof CN.Effects.sound === 'function') CN.Effects.sound('join');
      });
    }
    runtime.activePlayerIds = nextActiveIds;
    runtime.playersSnapshotReady = true;
  ...
  try {
    ...
    for (let attempt = 0; attempt < 5 && !created; attempt += 1) {
      ...
      const room = {
        ...
        entry: { rulesVersion: 17, status: 'lobby', roomName },
...
function modernGuestPlayer(name) {
  return {
    uid: runtime.uid,
    name,
    team: 'spectator',
...
function legacyGuestPlayer(name) {
  // V13-compatible payload. It is deliberately used only as a fallback when
  // the deployed Realtime Database rules have not yet been upgraded.
  // حمولة متوافقة مع V13 كي لا يتعطل دخول الضيف إذا بقيت القواعد القديمة منشورة.
  return {
    uid: runtime.uid,
    name,
    team: 'unassigned',
...
async function readEntryMetadataBestEffort() {
  try {
    const snap = await get(roomRef('entry'));
    if (snap.exists()) {
      const value = snap.val() || {};
      runtime.roomRulesVersion = Number(value.rulesVersion || 0) || null;
      return Object.assign({ exists: true }, value);
    }
    // V17 rules also allow authenticated visitors to read the non-secret room
    // status. This keeps upgraded V16 rooms joinable even though they do not
    // yet contain the new /entry node.
    const statusSnap = await get(roomRef('status'));
    if (statusSnap.exists()) return { exists: true, status: statusSnap.val(), rulesVersion: runtime.roomRulesVersion };
    return { exists: false };
  } catch (error) {
    // Old rules do not expose /entry or /status to visitors. This is expected
    // during migration, so do not fail the join solely because this probe was denied.
    // القواعد القديمة لا تسمح بقراءة بيانات الدخول قبل الانضمام؛ لا نوقف المحاولة بسبب ذلك.
    if (!isPermissionDenied(error)) console.warn('Invite entry probe failed:', error);
    return null;
  }
}

async function createGuestPlayerWithCompatibility(name) {
  const myRef = roomRef(`players/${runtime.uid}`);
  let existing = null;
  try {
    existing = await get(myRef);
  } catch (error) {
    // Some older/locked rules may deny the pre-read but still allow the
    // creation write. Continue and let the actual write decide.
  ...
  if (existing && existing.exists()) {
    ...
    if (!existingValue.avatarId) {
      try { await set(roomRef(`players/${runtime.uid}/avatarId`), (Date.now() % 25) + 1); } catch (_) { /* legacy rules may not know avatarId */ }
    }
    runtime.joinCompatibilityMode = existingValue.team === 'unassigned' ? 'legacy-v13' : 'modern';
    return { mode: runtime.joinCompatibilityMode, restored: true };
  }

  try {
    await set(myRef, modernGuestPlayer(name));
    runtime.joinCompatibilityMode = 'modern';
    return { mode: 'modern', restored: false };
  } catch (modernError) {
    if (!isPermissionDenied(modernError)) throw modernError;
    console.warn('Modern spectator join was denied; trying V13-compatible lobby join.', modernError);
    try {
      await set(myRef, legacyGuestPlayer(name));
      runtime.joinCompatibilityMode = 'legacy-v13';
      return { mode: 'legacy-v13', restored: false };
    } catch (legacyError) {
      legacyError.modernJoinError = modernError;
      throw legacyError;
    }
  }
}
...
async function joinRoom() {
  ...
  try {
    ...
    saveName(name);
    runtime.roomCode = code;
    const entryMeta = await readEntryMetadataBestEffort();
    if (entryMeta && entryMeta.exists === false) {
      throw Object.assign(new Error('ROOM_NOT_FOUND'), { code: 'ROOM_NOT_FOUND' });
    }
    if (entryMeta && entryMeta.exists !== false && !['lobby','playing'].includes(String(entryMeta.status || ''))) {
      throw Object.assign(new Error('ROOM_CLOSED'), { code: 'ROOM_CLOSED' });
    }
    const joined = await createGuestPlayerWithCompatibility(name);
    if (joined.mode === 'legacy-v13' && entryMeta && entryMeta.status === 'playing') {
      // Old V13 rules can only create a player while status=lobby. If this ever
      // happens due to a race, fail clearly instead of showing a false success.
      throw Object.assign(new Error('Legacy rules cannot join an active match.'), { code: 'LEGACY_RULES_ACTIVE_MATCH' });
    }
    ...
    runtime.roomCode = null;
    const codeText = String(error && error.code || '');
    let message = friendlyFirebaseError(error, 'Room not found, room was closed, or access is unavailable.');
    if (codeText.includes('ROOM_NOT_FOUND')) {
      message = 'Room not found. Ask the host for a fresh invite link.';
    } else if (codeText.includes('ROOM_CLOSED')) {
      message = 'This room is closed.';
    } else if (codeText.includes('LEGACY_RULES_ACTIVE_MATCH')) {
      message = 'The room is active, but Firebase is still using old rules. Publish the firebase-rules.json included with V18 (same join rules as V17) to allow joining during a game.';
    } else if (isPermissionDenied(error)) {
      message = 'Firebase رفض طلب الدخول. غالباً قواعد Realtime Database المنشورة قديمة أو App Check يمنع الطلب. انشر firebase-rules.json الموجود مع V18 (وهو نفس مخطط دخول V17)، وتأكد أن Anonymous Authentication مفعّل. / Firebase denied the join. Publish the database rules included with V18 (same join schema as V17) and verify Anonymous Auth; if App Check is enforced, configure it or disable enforcement.';
    }
    setMessage(messageTarget, message);
  } finally {
...
async function hostApplyAssignment(uid, team, role) {
  const player = runtime.players[uid];
  if (!runtime.isHost || !player) return { ok: false, code: 'HOST_ONLY' };
  let nextTeam = team;
  let nextRole = role;
  if (team === 'spectator' || role === 'spectator' || team === 'unassigned') {
    nextTeam = 'spectator';
    nextRole = 'spectator';
  }
  ...
  if (runtime.roomStatus === 'lobby' && runtime.roomSettings.teamsLocked && uid !== runtime.uid) return { ok: false, code: 'TEAMS_LOCKED' };
  const patch = {};
  if (nextRole === 'spymaster') {
    Object.values(runtime.players).forEach((other) => {
      if (other && other.uid !== uid && other.team === nextTeam && other.role === 'spymaster') {
        patch[`players/${other.uid}/role`] = 'operative';
      }
    });
  }
  patch[`players/${uid}/team`] = nextTeam;
  patch[`players/${uid}/role`] = nextRole;
...
async function setSelfAssignment(team, role) {
  ...
  if (runtime.isHost) return hostApplyAssignment(runtime.uid, nextTeam, nextRole);
  try {
    await update(roomRef(`players/${runtime.uid}`), { team: nextTeam, role: nextRole });
    return { ok: true };
  } catch (error) {
    if (!isPermissionDenied(error)) throw error;
    // V13 rules let a joined guest create actions but do not let that guest
    // edit team/role directly. Ask the host to perform the assignment.
    // قواعد V13 تمنع تعديل الفريق مباشرة؛ لذلك نطلب من المضيف تنفيذ التغيير.
    const result = await requestAction('assignSelf', { team: nextTeam, role: nextRole }, { allowLobby: true });
    return result;
  }
}

async function hostApplyProfile(uid, name, avatarId) {
  if (!runtime.isHost || !runtime.players[uid] || !['lobby','playing'].includes(runtime.roomStatus)) return { ok: false, code: 'HOST_ONLY' };
  const clean = normalizeName(name);
  if (!clean) return { ok: false, code: 'INVALID_NAME' };
  const id = Math.max(1, Math.min(25, Math.trunc(Number(avatarId) || 1)));
  await update(roomRef(`players/${uid}`), { name: clean, avatarId: id });
  return { ok: true, name: clean, avatarId: id };
}
...
async function savePlayerProfile(name, avatarId) {
  if (!runtime.inRoom || !['lobby', 'playing'].includes(runtime.roomStatus)) return { ok: false, code: 'NOT_IN_ROOM' };
  const clean = normalizeName(name);
  if (!clean) return { ok: false, code: 'INVALID_NAME' };
  const id = Math.max(1, Math.min(25, Math.trunc(Number(avatarId) || 1)));
  let result = { ok: true, name: clean, avatarId: id };
  try {
    // Modern rules allow the player to atomically update both profile fields.
    await update(roomRef(`players/${runtime.uid}`), { name: clean, avatarId: id });
  } catch (error) {
    if (!isPermissionDenied(error)) throw error;
    // Older deployed rules can reject a player-level multi-field update.
    // Route the same atomic profile change through the room host instead.
    result = await requestAction('saveProfile', { name: clean, avatarId: id }, { allowLobby: true });
    if (!result || result.ok === false) return result || { ok: false, code: 'SAVE_FAILED' };
  }
  saveName(clean);
  if (runtime.players[runtime.uid]) Object.assign(runtime.players[runtime.uid], { name: clean, avatarId: id });
  if (runtime.player) Object.assign(runtime.player, { name: clean, avatarId: id });
  renderShell();
  return { ok: true, name: clean, avatarId: id };
}
...
function getInviteUrl() { return inviteUrl(); }
function focusLobby() {
  if (!runtime.inRoom || runtime.roomStatus !== 'lobby') return { ok: false, code: 'NOT_IN_LOBBY' };
  CN.UI.setRuntimeMode('online');
  renderShell();
  requestAnimationFrame(() => { if (dom.lobbySection) { dom.lobbySection.hidden = false; dom.lobbySection.scrollIntoView({ block: 'start' }); } });
  return { ok: true };
}
...
async function startOnlineGame() {
  ...
  setMessage(dom.lobbyValidationMessage, '');
  const players = Object.values(runtime.players).filter((p) => p && p.online !== false).map((p) => {
    const spectator = p.team === 'spectator' || p.team === 'unassigned' || p.role === 'spectator';
    return { id: p.uid, name: p.name, team: spectator ? 'spectator' : p.team, role: spectator ? 'spectator' : p.role };
  });
...
function authorizeAction(action) {
  ...
  if (action.type === 'assignSelf') {
    if (!['lobby','playing'].includes(runtime.roomStatus)) return { ok: false, code: 'ROOM_NOT_OPEN' };
    if (runtime.roomStatus === 'lobby' && runtime.roomSettings.teamsLocked) return { ok: false, code: 'TEAMS_LOCKED' };
...
async function processOneAction(actionId, action) {
  ...
  try {
    ...
    const roomManagementOnly = action.type === 'assignSelf' || action.type === 'saveProfile';
    if (!roomManagementOnly) {
    ...
    if (authz.ok) {
      ...
      switch (action.type) {
        case 'assignSelf': result = await hostApplyAssignment(action.uid, payload.team, payload.role); break;
        case 'saveProfile': result = await hostApplyProfile(action.uid, payload.name, payload.avatarId); break;
        case 'guess': result = CN.Game.handleGuess(payload.cardId, { actorId: action.uid, actorName: authz.player.name }); break;
        case 'submitClue': result = CN.Game.submitClue(Object.assign({}, payload, { actorId: action.uid, actorName: authz.player.name })); break;
        case 'endTurn': result = CN.Game.endTurn('VOLUNTARY', { actorId: action.uid, actorName: authz.player.name }); break;
      ...
      if (result.ok && !['resetRound', 'returnLobby', 'assignSelf', 'saveProfile'].includes(action.type)) await broadcastHostState(false);
    }
...
function maybeStartActionProcessor() {
  const ready = runtime.isHost && runtime.inRoom && (runtime.roomStatus === 'lobby' || (runtime.roomStatus === 'playing' && runtime.publicGame && runtime.secretMap));
  if (!ready) {
...
async function requestAction(type, payload, options = {}) {
  const allowLobby = Boolean(options && options.allowLobby);
  if (!runtime.inRoom || (runtime.roomStatus !== 'playing' && !(allowLobby && runtime.roomStatus === 'lobby'))) return { ok: false, code: 'ROOM_NOT_PLAYING' };
  ...
  try {
    ...
    window.Codenames.Online = {
      ...
      getContext: () => ({
        ...
        players: Object.values(runtime.players || {}).filter(Boolean).map((player) => ({ ...player })),
        roomSettings: { ...runtime.roomSettings },
        joinCompatibilityMode: runtime.joinCompatibilityMode,
        roomRulesVersion: runtime.roomRulesVersion
      }),
      setSelfAssignment,
      resetTeams,
      randomizeTeams,
