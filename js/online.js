...
function setInviteLanding(active, code) {
  const isInvite = Boolean(active && code);
  if (dom.createRoomEntry) dom.createRoomEntry.hidden = isInvite;
  if (dom.joinRoomEntry) dom.joinRoomEntry.classList.toggle('invite-join-entry', isInvite);
  if (dom.joinRoomCodeField) dom.joinRoomCodeField.hidden = isInvite;
  if (dom.inviteJoinHelp) dom.inviteJoinHelp.hidden = !isInvite;
  if (dom.joinRoomTitle) dom.joinRoomTitle.textContent = isInvite ? '👋 اكتب اسمك للانضمام / Enter your name to join' : '🔗 Join Room';
  if (dom.joinRoomBtn) dom.joinRoomBtn.textContent = isInvite ? 'انضمام / Join' : 'Join Room';
  if (isInvite && dom.joinRoomCodeInput) dom.joinRoomCodeInput.value = code;
}
    ...
    if (authz.ok) {
      ...
      switch (action.type) {
        ...
        case 'commitCandidate': {
          result = CN.Game.commitCandidate(payload.cardId, action.uid);
          // Recover from a stale/lost candidate snapshot: the confirm button itself proves client intent.
          // استعادة آمنة إذا ضاع التحديد المؤقت لحظياً أثناء المزامنة.
          if (!result.ok && result.code === 'CANDIDATE_NOT_SELECTED') {
            const recovery = CN.Game.toggleCandidate(payload.cardId, action.uid);
            if (recovery.ok) result = CN.Game.commitCandidate(payload.cardId, action.uid);
          }
          break;
        }
...
async function boot() {
  try {
    ...
    if (roomParam) {
      ...
      setTimeout(() => dom.joinNameInput && dom.joinNameInput.focus(), 0);
    } else {
