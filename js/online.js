...
async function joinRoom() {
  ...
  try {
    ...
    if (codeText.includes('ROOM_NOT_FOUND')) {
      ...
      message = 'The room is active, but Firebase is still using old rules. Publish the V17 firebase-rules.json to allow joining during a game.';
    } else if (isPermissionDenied(error)) {
      message = 'Firebase رفض طلب الدخول. غالباً قواعد Realtime Database المنشورة قديمة أو App Check يمنع الطلب. انشر firebase-rules.json من V17، وتأكد أن Anonymous Authentication مفعّل. / Firebase denied the join. Publish the V17 database rules and verify Anonymous Auth; if App Check is enforced, configure it or disable enforcement.';
    }
