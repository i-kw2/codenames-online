/*
 * Local QR encoder bundled for Codenames 0.2.2v.
 * Based on Kazuhiko Arase's QRCode for JavaScript (MIT License),
 * as vendored by qrcode-terminal. Kept local so invite QR generation
...
(function (global) {
  ...
  function normalize(path) {
    const out = [];
    path.split('/').forEach((part) => { if (!part || part === '.') return; if (part === '..') out.pop(); else out.push(part); });
    return out.join('/');
  }
  ...
  function requireModule(id, from) {
    const resolved = from ? resolve(from, id) : normalize(id);
    const record = modules[resolved];
    if (!record) throw new Error('QR module not found: ' + resolved);
    if (!record.loaded) {
      record.loaded = true;
      const module = { exports: record.exports };
      record.factory(module, module.exports, (request) => requireModule(request, resolved));
...
module.exports = QRCode;
  ...
  const QRCodeCore = requireModule('index');
  const QRErrorCorrectLevel = requireModule('QRErrorCorrectLevel');

  function makeMatrix(text, level) {
    const levels = QRErrorCorrectLevel;
    const errorLevel = levels[String(level || 'M').toUpperCase()] || levels.M || levels.L;
    const qr = new QRCodeCore(-1, errorLevel);
  ...
  function render(target, text, options) {
    ...
    if (!node) throw new Error('QR target not found.');
    const opts = Object.assign({ size: 236, margin: 4, level: 'M', ariaLabel: 'Invite QR code' }, options || {});
    const matrix = makeMatrix(text, opts.level);
    const count = matrix.length;
    const full = count + opts.margin * 2;
    ...
    white.setAttribute('width', String(full)); white.setAttribute('height', String(full)); white.setAttribute('fill', '#fff');
    svg.appendChild(white);
    let d = '';
    for (let y = 0; y < count; y += 1) {
      for (let x = 0; x < count; x += 1) {
        if (matrix[y][x]) d += `M${x + opts.margin},${y + opts.margin}h1v1h-1z`;
      }
    }
    ...
    return { svg, matrix, text: String(text || '') };
  }

  global.CodenamesQRCode = { makeMatrix, render };
