(function (global) {
  ...
  function translateSecondaryUi() {
    ...
    if(refs.newsDialogV14){const h=refs.newsDialogV14.querySelector('h2'),p=refs.newsDialogV14.querySelector('p'); if(h)h.textContent=q('News','الأخبار'); if(p)p.textContent=q('V17 fixes Firebase invite-join compatibility, falls back to the V13 lobby join schema when old rules are still deployed, adds clearer permission diagnostics, and keeps the Welcome to Codenames gate and V13 game engine intact.','يصلح V17 توافق الدخول من رابط الدعوة مع Firebase، ويجرب مخطط V13 القديم كحل احتياطي عند بقاء القواعد القديمة، ويعرض تشخيصاً أوضح للصلاحيات مع الحفاظ على بوابة Welcome ومحرك V13.');}
  }
