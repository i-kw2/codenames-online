(function (global) {
  ...
  const strings = {
    en: {
      ...
      gameSettings:'GAME SETTINGS', classic:'CLASSIC', players4:'4+ PLAYERS', wordLang:'WORD PACKS & LANGUAGE', timer:'TIMER', off:'OFF', reset:'Reset teams', qrInvite:'QR Invite', random:'Randomize teams', start:'START GAME', waiting:'WAITING FOR HOST',
    ...
    ar: {
      ...
      gameSettings:'إعدادات اللعبة', classic:'كلاسيك', players4:'4+ لاعبين', wordLang:'حزم الكلمات واللغة', timer:'المؤقت', off:'متوقف', reset:'إعادة الفرق', qrInvite:'باركود الدعوة', random:'توزيع الفرق عشوائياً', start:'ابدأ اللعبة', waiting:'بانتظار المضيف',
  ...
  function cache() {
    [
      ...
      'blueOperativesList','blueSpymasterList','redOperativesList','redSpymasterList','gameSettingsTitle','wordLanguageRow','wordLanguageSummary','resetTeamsBtn','showQrInviteBtn','onlineRandomTeamsBtn','onlineStartGameBtn','lobbyValidationMessage',
      'qrInviteDialog','qrInviteCloseBtn','qrInviteTitle','qrInviteHelp','qrInviteCanvas','qrInviteUrlText','qrInviteCopyBtn','roomSettingsDialog','roomSettingsCloseBtn','settingsPlayersList','settingsLockTeams','settingsResetTeamsBtn','settingsRandomTeamsBtn','settingsCardLanguage','settingsStrictClues','settingsExpertRules','settingsCopyInviteBtn',
      'settingsAvatarPreview','settingsNickname','settingsAvatarSelect','settingsLeaveMatchBtn','settingsSavePlayerBtn','settingsSaveMessage','prefUiLanguage','prefMusicVolume','prefEffectsVolume','prefAlwaysGuess','prefHideRoomUrl','prefShowHintsBtn',
  ...
  function translateLobbyShell() {
    ...
    if(refs.showQrInviteBtn) refs.showQrInviteBtn.textContent=t('qrInvite');
  ...
  function translateSecondaryUi() {
    ...
    if(refs.settingsSavePlayerBtn && !/Saved|تم الحفظ|Saving|جار/.test(refs.settingsSavePlayerBtn.textContent))refs.settingsSavePlayerBtn.textContent=q('Save changes','حفظ التعديلات');
    ...
    if(refs.newsDialogV14){const h=refs.newsDialogV14.querySelector('h2'),p=refs.newsDialogV14.querySelector('p'); if(h)h.textContent=q('News','الأخبار'); if(p)p.textContent=q('0.2.2v adds a lobby QR invite, reliable nickname/avatar saving with a lobby return, a join chime, and the new blurred shared background artwork while preserving the stable realtime game flow.','يضيف الإصدار 0.2.2v باركود دعوة داخل اللوبي، وحفظاً موثوقاً للاسم والصورة مع الرجوع إلى اللوبي، وصوتاً عند دخول عضو جديد، والخلفية الجديدة مع Blur، مع الحفاظ على استقرار اللعب والـRealtime.');}
  }
  ...
  function renderLobby() {
    ...
    if (refs.showQrInviteBtn) refs.showQrInviteBtn.disabled = !inLobby;
  ...
  function renderQrInviteText() {
    const ar = lang() === 'ar';
    if (refs.qrInviteTitle) refs.qrInviteTitle.textContent = ar ? 'امسح الباركود للدخول' : 'Scan to join room';
    if (refs.qrInviteHelp) refs.qrInviteHelp.textContent = ar
      ? 'امسح هذا الباركود من الهاتف. سيفتح أولاً شاشة اختيار الاسم ثم يدخلك إلى نفس لوبي الغرفة.'
      : 'Scan this code on a phone. It opens the nickname gate first, then joins this room lobby.';
    if (refs.qrInviteCopyBtn && !/Copied|تم النسخ/.test(refs.qrInviteCopyBtn.textContent)) refs.qrInviteCopyBtn.textContent = ar ? 'نسخ رابط الدعوة' : 'Copy invite link';
  }

  function openQrInvite() {
    const api = onlineApi();
    if (!api || !api.getInviteUrl || !context.inRoom || context.roomStatus !== 'lobby') return;
    const url = api.getInviteUrl();
    renderQrInviteText();
    if (refs.qrInviteUrlText) refs.qrInviteUrlText.textContent = url;
    if (refs.qrInviteCanvas) {
      refs.qrInviteCanvas.replaceChildren();
      try {
        if (!global.CodenamesQRCode || !global.CodenamesQRCode.render) throw new Error('QR renderer unavailable');
        global.CodenamesQRCode.render(refs.qrInviteCanvas, url, { size: 248, margin: 4, level: 'M', ariaLabel: lang()==='ar' ? 'باركود رابط دعوة الغرفة' : 'Room invite QR code' });
      } catch (error) {
        console.error('QR generation failed:', error);
        const fallback = document.createElement('p');
        fallback.className = 'qr-invite-error';
        fallback.textContent = lang()==='ar' ? 'تعذر إنشاء الباركود. استخدم رابط الدعوة بالأسفل.' : 'Could not generate the QR code. Use the invite link below.';
        refs.qrInviteCanvas.appendChild(fallback);
      }
    }
    showDialog(refs.qrInviteDialog);
  }
  ...
  function renderSettings() {
    ...
    const profileEditable = Boolean(context.inRoom && ['lobby','playing'].includes(context.roomStatus));
    refs.settingsNickname.disabled=!profileEditable; refs.settingsAvatarSelect.disabled=!profileEditable;
    if(refs.settingsSavePlayerBtn) refs.settingsSavePlayerBtn.disabled=!profileEditable;
    if(refs.settingsSaveMessage){ refs.settingsSaveMessage.hidden=true; refs.settingsSaveMessage.textContent=''; refs.settingsSaveMessage.classList.remove('error'); }
    refs.settingsAvatarPreview.replaceChildren(); const img=document.createElement('img'); img.src=avatarSrc({ ...p, avatarId: aid }); img.alt=''; refs.settingsAvatarPreview.appendChild(img);
  ...
  function bind() {
    ...
    refs.showQrInviteBtn&&refs.showQrInviteBtn.addEventListener('click',openQrInvite);
    refs.qrInviteCloseBtn&&refs.qrInviteCloseBtn.addEventListener('click',()=>hideDialog(refs.qrInviteDialog));
    refs.qrInviteCopyBtn&&refs.qrInviteCopyBtn.addEventListener('click',async()=>{
      const a=onlineApi(); if(!a||!a.copyInvite)return;
      await a.copyInvite();
      refs.qrInviteCopyBtn.textContent=lang()==='ar'?'تم النسخ ✓':'Copied ✓';
      setTimeout(()=>renderQrInviteText(),1200);
    });
    ...
    refs.settingsAvatarSelect&&refs.settingsAvatarSelect.addEventListener('change',()=>{
      if(!refs.settingsAvatarPreview)return;
      refs.settingsAvatarPreview.replaceChildren();
      const img=document.createElement('img'); img.src=avatarSrc({ ...(context.player||{}), avatarId:Number(refs.settingsAvatarSelect.value) }); img.alt=''; refs.settingsAvatarPreview.appendChild(img);
    });
    const savePlayerSettings=async()=>{
      const a=onlineApi(); if(!a||!refs.settingsSavePlayerBtn)return;
      const q=(en,arText)=>lang()==='ar'?arText:en;
      const original=q('Save changes','حفظ التعديلات');
      const name=String(refs.settingsNickname&&refs.settingsNickname.value||'').trim();
      if(!name){ if(refs.settingsSaveMessage){refs.settingsSaveMessage.textContent=q('Please enter a nickname.','اكتب اسماً أولاً.');refs.settingsSaveMessage.classList.add('error');refs.settingsSaveMessage.hidden=false;} return; }
      refs.settingsSavePlayerBtn.disabled=true; refs.settingsSavePlayerBtn.textContent=q('Saving…','جارٍ الحفظ…');
      if(refs.settingsSaveMessage){refs.settingsSaveMessage.hidden=true;refs.settingsSaveMessage.classList.remove('error');}
      try{
        let result;
        if(a.savePlayerProfile) result=await a.savePlayerProfile(name,Number(refs.settingsAvatarSelect.value));
        else { const r1=a.setNickname?await a.setNickname(name):{ok:false}; const r2=a.setAvatar?await a.setAvatar(Number(refs.settingsAvatarSelect.value)):{ok:false}; result={ok:r1&&r1.ok!==false&&r2&&r2.ok!==false}; }
        if(result&&result.ok===false) throw new Error(result.code||'SAVE_FAILED');
        refs.settingsSavePlayerBtn.textContent=q('Saved ✓','تم الحفظ ✓');
        if(refs.settingsSaveMessage){refs.settingsSaveMessage.textContent=q('Your changes were saved.','تم حفظ تعديلاتك.');refs.settingsSaveMessage.hidden=false;}
        setTimeout(()=>{
          if(refs.settingsSavePlayerBtn){refs.settingsSavePlayerBtn.textContent=original;refs.settingsSavePlayerBtn.disabled=false;}
          closeSettings();
          if(a.focusLobby && context.roomStatus==='lobby') a.focusLobby();
          else if(context.roomStatus==='lobby'){ const lobby=document.getElementById('lobbySection'); if(lobby){lobby.hidden=false; lobby.scrollIntoView({block:'start'});} }
        },420);
      }catch(error){
        console.error('Save player settings failed:',error);
        refs.settingsSavePlayerBtn.textContent=original; refs.settingsSavePlayerBtn.disabled=false;
        if(refs.settingsSaveMessage){refs.settingsSaveMessage.textContent=q('Could not save changes. Please try again.','تعذر حفظ التعديلات. حاول مرة أخرى.');refs.settingsSaveMessage.classList.add('error');refs.settingsSaveMessage.hidden=false;}
      }
    };
    refs.settingsSavePlayerBtn&&refs.settingsSavePlayerBtn.addEventListener('click',savePlayerSettings);
    refs.settingsNickname&&refs.settingsNickname.addEventListener('keydown',(event)=>{if(event.key==='Enter'){event.preventDefault();savePlayerSettings();}});
