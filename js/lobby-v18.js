(function (global) {
  ...
  function translateSecondaryUi() {
    ...
    if(refs.newsDialogV14){const h=refs.newsDialogV14.querySelector('h2'),p=refs.newsDialogV14.querySelector('p'); if(h)h.textContent=q('News','الأخبار'); if(p)p.textContent=q('V18 keeps the stable V17 invite and realtime flow, then adds the full in-game presentation: a bottom-center 25-card deal, dedicated Operative and Spymaster interfaces, a synchronized 3-second clue announcement, and a shared live Game Log.','يحافظ V18 على دخول الدعوات والـRealtime المستقر من V17، ويضيف واجهة اللعب الكاملة: توزيع 25 بطاقة من منتصف الأسفل، واجهتي Operative وSpymaster، إعلان تلميح متزامن لمدة 3 ثوانٍ، وGame Log مشترك يظهر للجميع.');}
  }
  ...
  function bind() {
    ...
    const savePlayerSettings=async()=>{
      ...
      try{
        ...
        setTimeout(()=>{ if(refs.settingsSavePlayerBtn){refs.settingsSavePlayerBtn.textContent=original;refs.settingsSavePlayerBtn.disabled=false;} closeSettings(); },420);
      }catch(error){
