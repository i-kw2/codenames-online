(function (global) {
  ...
  function sound(name) {
    ...
    switch (name) {
      ...
      case 'join': tone(587,.07,'sine',.08); tone(784,.09,'sine',.09,.055); tone(988,.12,'sine',.075,.12); break;
  ...
  function reducedMotion() {
    const manual = Boolean(document.body && document.body.classList.contains('disable-gameplay-animations'));
    return manual || Boolean(global.matchMedia && global.matchMedia('(prefers-reduced-motion: reduce)').matches);
  }
  ...
  async function dealBoardCards(generation) {
    if (generation !== overlayGeneration) return;
    const board = document.getElementById('board');
    if (!board) return;
    const cards = Array.from(board.querySelectorAll('.card-button'));
    if (cards.length !== 25) return;

    if (reducedMotion() || !Element.prototype.animate) {
      board.classList.remove('pending-deal-v18');
      board.classList.add('deal-complete-v18');
      return;
    }

    const originX = global.innerWidth / 2;
    const originY = global.innerHeight + 44;
    const duration = 560;
    const stagger = 72;
    const animations = [];

    document.body.classList.add('deal-animation-active');
    board.classList.add('dealing-v18');
    board.inert = true;
    ...
    cards.forEach((card, index) => {
      ...
      const anim = card.animate([
        { transform: `translate(${dx}px, ${dy}px) scale(.97)`, offset: 0 },
        { transform: 'translate(0, -4px) scale(1)', offset: .88 },
    ...
    await wait((cards.length - 1) * stagger + duration + 30);
    animations.forEach((animation) => { try { animation.cancel(); } catch (_) { /* ignore */ } });
    board.classList.remove('dealing-v18');
    board.classList.add('deal-complete-v18');
    board.inert = false;
    board.removeAttribute('aria-busy');
    document.body.classList.remove('deal-animation-active');
  }

  function removeClueOverlays() {
    document.querySelectorAll('.clue-reveal-overlay-v18').forEach((node) => node.remove());
    document.body.classList.remove('clue-reveal-active', 'clue-reveal-pending');
    const board = document.getElementById('board');
    if (board && !document.body.classList.contains('deal-animation-active')) {
      board.inert = false;
      board.removeAttribute('aria-busy');
    }
  }

  async function showClueReveal(payload, generation) {
    if (generation !== overlayGeneration || !payload) return;
    removeClueOverlays();
    const count = payload.mode === 'unlimited' ? '∞' : payload.mode === 'zero' ? '0' : String(payload.number == null ? '' : payload.number);
    const layer = document.createElement('div');
    layer.className = `clue-reveal-overlay-v18 team-${payload.team || 'neutral'}`;
    layer.setAttribute('role', 'status');
    layer.setAttribute('aria-live', 'assertive');
    layer.setAttribute('aria-atomic', 'true');

    const card = document.createElement('div');
    card.className = 'clue-reveal-card-v18';
    const word = document.createElement('strong');
    word.className = 'clue-reveal-word-v18';
    word.textContent = payload.text || '';
    const number = document.createElement('span');
    number.className = 'clue-reveal-count-v18';
    number.textContent = count;
    card.append(word, number);
    layer.appendChild(card);
    document.body.appendChild(layer);

    const board = document.getElementById('board');
    document.body.classList.remove('clue-reveal-pending');
    document.body.classList.add('clue-reveal-active');
    if (board) {
      board.inert = true;
      board.setAttribute('aria-busy', 'true');
    }
    ...
    if (reducedMotion()) {
      layer.classList.add('show', 'reduced-motion');
      await wait(3000);
    } else {
  ...
  function requestInitialDeal(state) {
    if (!state || !state.roundId || lastDealRoundId === state.roundId) return;
    const immediateBoard = document.getElementById('board');
    if (immediateBoard && immediateBoard.querySelectorAll('.card-button').length === 25) immediateBoard.classList.add('pending-deal-v18');
    const snapshot = {
      roundId: state.roundId,
      createdAt: state.createdAt,
      logCount: Array.isArray(state.logs) ? state.logs.length : 0
    };
    lastDealRoundId = state.roundId;

    // Defer until the Game Effects subscriber has processed the new-round log and
    // advanced its presentation generation. UI renders before Effects on each notify.
    setTimeout(() => {
      if (lastDealRoundId !== snapshot.roundId) return;
      const board = document.getElementById('board');
      if (!board || board.querySelectorAll('.card-button').length !== 25) return;

      // A late joiner should see the current board immediately rather than replaying the opening deal.
      const onlineMode = CN.UI && CN.UI.getRuntimeMode && CN.UI.getRuntimeMode() === 'online';
      const created = Date.parse(snapshot.createdAt || '');
      const tooOld = Number.isFinite(created) && Math.abs(Date.now() - created) > 8000;
      const hasProgress = snapshot.logCount > 2;
      if (onlineMode && (tooOld || hasProgress)) {
        board.classList.remove('pending-deal-v18');
        board.classList.add('deal-complete-v18');
        return;
      }
      queueOverlay((generation) => dealBoardCards(generation));
    }, 0);
  }
  ...
  function processEvent(entry) {
    ...
    if (entry.type === 'CLUE_SUBMITTED') {
      const board = document.getElementById('board');
      document.body.classList.add('clue-reveal-pending');
      if (board) { board.inert = true; board.setAttribute('aria-busy', 'true'); }
      queueOverlay((generation) => showClueReveal(p, generation));
      return;
    }
  ...
  CN.Effects = { init, sound, flash, confetti, updateControls, dealBoardCards, requestInitialDeal, showClueReveal, isClueRevealActive: () => document.body.classList.contains('clue-reveal-active') };
})(window);
